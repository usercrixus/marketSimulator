...
void MarketMakerAgent::onEpoch(Statistics &statistics, Market &market)
{
    ...
    Order buyOrder = Order::makePostOnlyLimit(-1, Order::Side::BUY, quantity, bidPrice, this);
    Order sellOrder = Order::makePostOnlyLimit(-1, Order::Side::SELL, quantity, askPrice, this);
    market.submitOrder(buyOrder);
    market.submitOrder(sellOrder);
    previousAsset = asset;
}

void MarketMakerAgent::onReward()
{
    if (!isUpdated)
        return;
